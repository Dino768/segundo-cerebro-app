...
- **Siguiente fase: zona de estudio** (sección de estudio con chat; ver `proyectos/zona-de-estudio.md` e `ideas/bandeja.md`). **Brainstorming empezado el 2026-09-24** (clasificado como arquitectónico: proceso completo). Diego ya confirmó este entendimiento:
  - Pestaña nueva «Estudio» en la barra lateral.
  - Un chat con Claude para pedir ejercicios, cómo se hacen cosas, resúmenes… Respuestas bien explicadas, con ejemplos, fórmulas y gráficos según haga falta.
  - Además del chat, una «pizarra blanca»: un lienzo amplio donde Claude reparte explicaciones, esquemas, fórmulas y dibujos por todas partes, no solo hacia abajo.
  - Supuestos confirmados: es para las asignaturas de la uni (encaja con `estudios/`, vacía) y lo estudiado se guarda en `my-context` para repasar.
  - **Decisión clave tomada (2026-09-24): nada de API de pago.** Diego eligió:
    - En el PC y el portátil la app funciona con un **programa local** (tipo `npm run dev`) que lee y escribe en su disco.
    - La zona de estudio es **chat + pizarra**. El chat es **de verdad**: Diego escribe en la app y por detrás contesta **Claude Code** con su suscripción. Hay que comprobar que esto es posible. Las conversaciones son las de Claude Code guardadas en local y **solo se ven en el PC o el portátil**, no en el móvil ni en la web publicada.
    - La **pizarra** solo se usa cuando Diego pide que le explique algo o pide un esquema (nunca un resumen automático).
  ...
  - **Siguiente:** que Diego revise el spec escrito. Cuando lo apruebe, escribir el plan con `superpowers:writing-plans`. Los pulidos pendientes ya están en el spec (sección 10):
